...
可安装在 Windows 和 macOS 的 X 大V监控桌面应用。发现新帖后自动完成中文翻译、观点逻辑分析、事实与数值主张拆解和免费网页证据检索。实时通知通过企业微信自建应用进入个人微信，企业微信群机器人只发送每日汇总，完整历史始终保存在桌面应用。
...
- 企业微信自建应用两阶段推送：Text 实时快讯 + MPNews 完整图文报告。
- 群机器人按指定时区和时间发送每日汇总，不逐条刷屏；成功日期会去重。
...
2. 企业微信自建应用的 CorpID、AgentID、Secret 和接收成员 UserID。
3. 企业微信群机器人 Webhook，以及每日汇总时间。
4. X Developer App 的 App-only Bearer Token；若不使用官方 API，再填写专用采集账号的 Cookie Header（必须含 `auth_token` 与 `ct0`）。
5. 时区、调用上限及是否开机自动运行。

接收成员必须在自建应用的可见范围内，并在企业微信“我的企业 → 微信插件”中关注插件、允许微信接收应用消息。点击“测试个人微信 Text + MPNews”会依次发送一条文本和一条图文，用于验证完整链路。

分别执行采集、模型、个人微信和群汇总测试，全部成功后添加监控账号并点击“启动监控”。应用窗口关闭后仍保留在系统托盘运行；通过托盘菜单可退出。
...
- 敏感配置通过 Electron `safeStorage` 使用 Windows DPAPI 或 macOS Keychain 支持的加密能力；API Key、自建应用 Secret、Webhook、Bearer Token 与 Cookie 不写入日志或普通数据文件。
...
- 企业微信 Text、MPNews、微信插件展示及每日汇总必须使用真实企业凭据做端到端测试；无凭据环境只验证请求结构、字节限制和错误处理。
